...
CREATE TRIGGER SToCKs AFTER INSERT 
ON Orders FOR EACH ROW
BEGIN
    UPDATE Stock 
    SET Stock.quantity = Stock.quantity - updated.order_quantity
    WHERE Stock.product_id = updated.product_id;
END $$
...
FROM Customers c
JOIN Orders o ON c.CustomerID = o.CustomerID -- c → label for Customers; o → label for Orders
JOIN OrderDetails od ON o.OrderID = od.OrderID -- od → label for OrderDetails
...
SELECT c.CustomerID, c.Name AS CustomerName,
    SUM(od.Price * od.Quantity) AS TotalSpent,
    RANK() OVER (ORDER BY SUM(od.Price * od.Quantity) DESC) AS SpendingRank
FROM Customers c
...
select monthname(OrderDate), count(orderId) FROM Orders 
